...
ThreeGppAntennaArrayModel::GetElementFieldPattern (Angles a) const
{
  ...
  a.SetAzimuth(fmod (a.GetAzimuth() + M_PI, 2 * M_PI));
  if (a.GetAzimuth() < 0)
      a.SetAzimuth(a.GetAzimuth() + M_PI);
  else 
      a.SetAzimuth(a.GetAzimuth() - M_PI);

  NS_ASSERT_MSG (a.GetInclination() >= 0 && a.GetInclination() <= M_PI, "The vertical angle should be between 0 and M_PI");
  NS_ASSERT_MSG (a.GetAzimuth() >= -M_PI && a.GetAzimuth() <= M_PI, "The horizontal angle should be between -M_PI and M_PI");
  ...
  NS_LOG_DEBUG (a.GetInclination() << " " << thetaPrime << " " << a.GetAzimuth() << " " << phiPrime);
  ...
  NS_LOG_DEBUG (a.GetAzimuth()/M_PI*180 << " " << a.GetInclination()/M_PI*180 << " " << fieldTheta*fieldTheta + fieldPhi*fieldPhi);
